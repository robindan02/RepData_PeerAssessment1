...
```{r Installing Package and obtain Data} 
library("data.table")
library(ggplot2)
...
```{r Calculate 5 minute interval steps}
...
```{r Calculate max steps per interval}
...
```{r Fill in median for missing steps}
...
```{r Read new dataset into data table}
...
```{r Total Steps per day, median and visual}
